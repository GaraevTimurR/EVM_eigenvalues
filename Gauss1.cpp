...
int Gauss(double* Matrix, double* X, double eps, double NormInf, int n){
	...
	double li=Matrix[0], Li, ui=Matrix[1]/li,  lyi;
	double Ui=0;
	int s;
	...
	for(int k=1; k<n+1; k++){
		...
		while(fabs(b-a)>eps*NormInf){
			...
			for(int i=1; i<n; i++){
				lyi=Matrix[i*n+i-1];
				Li=Matrix[i*n+i]-c-(lyi)*(ui);
				if(i!=n-1){
					Ui=Matrix[i+1+i*n]/Li;
				}
				if(Li/fabs(Li)<mach_eps){s++;} 
